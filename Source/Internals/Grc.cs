...
internal static unsafe class Grc
{
    ...
    public static bool CreateRuntimeTxd(string name, out pgDictionary<grcTexture>* txd, out strLocalIndex index)
    {
        txd = null;
        index = default;
        ...
        txdStore.AddRef(index, (strRefKind)2);
        Log($" > NumRefs = {txdStore.GetNumRefs(index)}");
        
        return true;
    }

    public static bool GetRuntimeTxd(string name, out pgDictionary<grcTexture>* txd, out strLocalIndex index)
    {
    ...
    public static void ReleaseTxd(string name, strLocalIndex index)
    {
        ...
        txdStore.RemoveRef(index, (strRefKind)2);
    ...
    public static grcRenderTarget* CreateRenderTargetTexture(string name, uint width, uint height, TextureFormat format)
    {
        if (!grcTextureFactory.Available)
        {
            Log("grcTextureFactory not available!");
        ...
        Log($"Creating render target texture (width:{width}, height:{height}, format:{format})");
        var createParams = grcTextureFactory.RenderTargetCreateParams.Default with
        {
            Format = (grcBufferFormat)format,
        };
        return grcTextureFactory.Instance.CreateRenderTarget(name, 3, width, height, 32, createParams);
    }
