namespace RAGENativeUI.Internals
{
    ...
    internal unsafe struct atArray<T> where T : unmanaged
    {
        ...
        public ref T Add()
        {
            if (Count == Size)
            {
                ...
                var newItems = sysMemAllocator.Current.Allocate(newByteSize, 16, 0);
                Buffer.MemoryCopy(Items, newItems, newByteSize, (ulong)(sizeof(T) * Size));
                sysMemAllocator.Current.Free(Items);
                Items = (T*)newItems;
                Size = (ushort)newSize;
            }
    ...
    internal unsafe struct atArrayOfPtrs<T> where T : unmanaged
    {
        ...
        public ref T* Add()
        {
            ...
            {
                var newSize = Size + 16;
                var newByteSize = (ulong)(sizeof(void*) * newSize);
                var newItems = sysMemAllocator.Current.Allocate(newByteSize, 16, 0);
                Buffer.MemoryCopy(Items, newItems, newByteSize, (ulong)(sizeof(void*) * Size));
                sysMemAllocator.Current.Free(Items);
                Items = (T**)newItems;
                Size = (ushort)newSize;
            }
        ...
        public ref struct Enumerator
        {
            private readonly atArrayOfPtrs<T>* array;
            private int index;

            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            public Enumerator(ref atArrayOfPtrs<T> arr)
            {
                array = (atArrayOfPtrs<T>*)Unsafe.AsPointer(ref arr);
                index = -1;
            }
