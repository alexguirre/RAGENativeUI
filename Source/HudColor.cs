namespace RAGENativeUI
{
    using System.Drawing;

    /// <summary>
    /// Defines identifiers that represent the game HUD colors.
    /// </summary>
    public enum HudColor
    {
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_PURE_WHITE".
        /// </summary>
        PureWhite = 0,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_WHITE".
        /// </summary>
        White = 1,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_BLACK".
        /// </summary>
        Black = 2,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_GREY".
        /// </summary>
        Grey = 3,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_GREYLIGHT".
        /// </summary>
        GreyLight = 4,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_GREYDARK".
        /// </summary>
        GreyDark = 5,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_RED".
        /// </summary>
        Red = 6,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_REDLIGHT".
        /// </summary>
        RedLight = 7,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_REDDARK".
        /// </summary>
        RedDark = 8,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_BLUE".
        /// </summary>
        Blue = 9,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_BLUELIGHT".
        /// </summary>
        BlueLight = 10,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_BLUEDARK".
        /// </summary>
        BlueDark = 11,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_YELLOW".
        /// </summary>
        Yellow = 12,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_YELLOWLIGHT".
        /// </summary>
        YellowLight = 13,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_YELLOWDARK".
        /// </summary>
        YellowDark = 14,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_ORANGE".
        /// </summary>
        Orange = 15,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_ORANGELIGHT".
        /// </summary>
        OrangeLight = 16,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_ORANGEDARK".
        /// </summary>
        OrangeDark = 17,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_GREEN".
        /// </summary>
        Green = 18,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_GREENLIGHT".
        /// </summary>
        GreenLight = 19,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_GREENDARK".
        /// </summary>
        GreenDark = 20,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_PURPLE".
        /// </summary>
        Purple = 21,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_PURPLELIGHT".
        /// </summary>
        PurpleLight = 22,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_PURPLEDARK".
        /// </summary>
        PurpleDark = 23,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_PINK".
        /// </summary>
        Pink = 24,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_RADAR_HEALTH".
        /// </summary>
        RadarHealth = 25,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_RADAR_ARMOUR".
        /// </summary>
        RadarArmour = 26,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_RADAR_DAMAGE".
        /// </summary>
        RadarDamage = 27,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_NET_PLAYER1".
        /// </summary>
        NetPlayer1 = 28,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_NET_PLAYER2".
        /// </summary>
        NetPlayer2 = 29,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_NET_PLAYER3".
        /// </summary>
        NetPlayer3 = 30,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_NET_PLAYER4".
        /// </summary>
        NetPlayer4 = 31,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_NET_PLAYER5".
        /// </summary>
        NetPlayer5 = 32,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_NET_PLAYER6".
        /// </summary>
        NetPlayer6 = 33,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_NET_PLAYER7".
        /// </summary>
        NetPlayer7 = 34,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_NET_PLAYER8".
        /// </summary>
        NetPlayer8 = 35,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_NET_PLAYER9".
        /// </summary>
        NetPlayer9 = 36,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_NET_PLAYER10".
        /// </summary>
        NetPlayer10 = 37,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_NET_PLAYER11".
        /// </summary>
        NetPlayer11 = 38,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_NET_PLAYER12".
        /// </summary>
        NetPlayer12 = 39,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_NET_PLAYER13".
        /// </summary>
        NetPlayer13 = 40,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_NET_PLAYER14".
        /// </summary>
        NetPlayer14 = 41,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_NET_PLAYER15".
        /// </summary>
        NetPlayer15 = 42,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_NET_PLAYER16".
        /// </summary>
        NetPlayer16 = 43,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_NET_PLAYER17".
        /// </summary>
        NetPlayer17 = 44,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_NET_PLAYER18".
        /// </summary>
        NetPlayer18 = 45,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_NET_PLAYER19".
        /// </summary>
        NetPlayer19 = 46,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_NET_PLAYER20".
        /// </summary>
        NetPlayer20 = 47,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_NET_PLAYER21".
        /// </summary>
        NetPlayer21 = 48,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_NET_PLAYER22".
        /// </summary>
        NetPlayer22 = 49,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_NET_PLAYER23".
        /// </summary>
        NetPlayer23 = 50,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_NET_PLAYER24".
        /// </summary>
        NetPlayer24 = 51,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_NET_PLAYER25".
        /// </summary>
        NetPlayer25 = 52,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_NET_PLAYER26".
        /// </summary>
        NetPlayer26 = 53,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_NET_PLAYER27".
        /// </summary>
        NetPlayer27 = 54,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_NET_PLAYER28".
        /// </summary>
        NetPlayer28 = 55,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_NET_PLAYER29".
        /// </summary>
        NetPlayer29 = 56,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_NET_PLAYER30".
        /// </summary>
        NetPlayer30 = 57,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_NET_PLAYER31".
        /// </summary>
        NetPlayer31 = 58,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_NET_PLAYER32".
        /// </summary>
        NetPlayer32 = 59,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_SIMPLEBLIP_DEFAULT".
        /// </summary>
        SimpleBlipDefault = 60,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_MENU_BLUE".
        /// </summary>
        MenuBlue = 61,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_MENU_GREY_LIGHT".
        /// </summary>
        MenuGreyLight = 62,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_MENU_BLUE_EXTRA_DARK".
        /// </summary>
        MenuBlueExtraDark = 63,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_MENU_YELLOW".
        /// </summary>
        MenuYellow = 64,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_MENU_YELLOW_DARK".
        /// </summary>
        MenuYellowDark = 65,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_MENU_GREEN".
        /// </summary>
        MenuGreen = 66,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_MENU_GREY".
        /// </summary>
        MenuGrey = 67,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_MENU_GREY_DARK".
        /// </summary>
        MenuGreyDark = 68,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_MENU_HIGHLIGHT".
        /// </summary>
        MenuHighlight = 69,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_MENU_STANDARD".
        /// </summary>
        MenuStandard = 70,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_MENU_DIMMED".
        /// </summary>
        MenuDimmed = 71,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_MENU_EXTRA_DIMMED".
        /// </summary>
        MenuExtraDimmed = 72,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_BRIEF_TITLE".
        /// </summary>
        BriefTitle = 73,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_MID_GREY_MP".
        /// </summary>
        MidGreyMultiplayer = 74,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_NET_PLAYER1_DARK".
        /// </summary>
        NetPlayer1Dark = 75,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_NET_PLAYER2_DARK".
        /// </summary>
        NetPlayer2Dark = 76,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_NET_PLAYER3_DARK".
        /// </summary>
        NetPlayer3Dark = 77,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_NET_PLAYER4_DARK".
        /// </summary>
        NetPlayer4Dark = 78,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_NET_PLAYER5_DARK".
        /// </summary>
        NetPlayer5Dark = 79,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_NET_PLAYER6_DARK".
        /// </summary>
        NetPlayer6Dark = 80,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_NET_PLAYER7_DARK".
        /// </summary>
        NetPlayer7Dark = 81,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_NET_PLAYER8_DARK".
        /// </summary>
        NetPlayer8Dark = 82,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_NET_PLAYER9_DARK".
        /// </summary>
        NetPlayer9Dark = 83,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_NET_PLAYER10_DARK".
        /// </summary>
        NetPlayer10Dark = 84,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_NET_PLAYER11_DARK".
        /// </summary>
        NetPlayer11Dark = 85,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_NET_PLAYER12_DARK".
        /// </summary>
        NetPlayer12Dark = 86,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_NET_PLAYER13_DARK".
        /// </summary>
        NetPlayer13Dark = 87,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_NET_PLAYER14_DARK".
        /// </summary>
        NetPlayer14Dark = 88,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_NET_PLAYER15_DARK".
        /// </summary>
        NetPlayer15Dark = 89,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_NET_PLAYER16_DARK".
        /// </summary>
        NetPlayer16Dark = 90,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_NET_PLAYER17_DARK".
        /// </summary>
        NetPlayer17Dark = 91,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_NET_PLAYER18_DARK".
        /// </summary>
        NetPlayer18Dark = 92,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_NET_PLAYER19_DARK".
        /// </summary>
        NetPlayer19Dark = 93,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_NET_PLAYER20_DARK".
        /// </summary>
        NetPlayer20Dark = 94,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_NET_PLAYER21_DARK".
        /// </summary>
        NetPlayer21Dark = 95,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_NET_PLAYER22_DARK".
        /// </summary>
        NetPlayer22Dark = 96,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_NET_PLAYER23_DARK".
        /// </summary>
        NetPlayer23Dark = 97,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_NET_PLAYER24_DARK".
        /// </summary>
        NetPlayer24Dark = 98,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_NET_PLAYER25_DARK".
        /// </summary>
        NetPlayer25Dark = 99,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_NET_PLAYER26_DARK".
        /// </summary>
        NetPlayer26Dark = 100,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_NET_PLAYER27_DARK".
        /// </summary>
        NetPlayer27Dark = 101,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_NET_PLAYER28_DARK".
        /// </summary>
        NetPlayer28Dark = 102,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_NET_PLAYER29_DARK".
        /// </summary>
        NetPlayer29Dark = 103,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_NET_PLAYER30_DARK".
        /// </summary>
        NetPlayer30Dark = 104,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_NET_PLAYER31_DARK".
        /// </summary>
        NetPlayer31Dark = 105,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_NET_PLAYER32_DARK".
        /// </summary>
        NetPlayer32Dark = 106,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_BRONZE".
        /// </summary>
        Bronze = 107,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_SILVER".
        /// </summary>
        Silver = 108,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_GOLD".
        /// </summary>
        Gold = 109,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_PLATINUM".
        /// </summary>
        Platinum = 110,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_GANG1".
        /// </summary>
        Gang1 = 111,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_GANG2".
        /// </summary>
        Gang2 = 112,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_GANG3".
        /// </summary>
        Gang3 = 113,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_GANG4".
        /// </summary>
        Gang4 = 114,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_SAME_CREW".
        /// </summary>
        SameCrew = 115,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_FREEMODE".
        /// </summary>
        Freemode = 116,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_PAUSE_BG".
        /// </summary>
        PauseBackground = 117,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_FRIENDLY".
        /// </summary>
        Friendly = 118,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_ENEMY".
        /// </summary>
        Enemy = 119,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_LOCATION".
        /// </summary>
        Location = 120,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_PICKUP".
        /// </summary>
        Pickup = 121,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_PAUSE_SINGLEPLAYER".
        /// </summary>
        PauseSingleplayer = 122,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_FREEMODE_DARK".
        /// </summary>
        FreemodeDark = 123,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_INACTIVE_MISSION".
        /// </summary>
        InactiveMission = 124,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_DAMAGE".
        /// </summary>
        Damage = 125,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_PINKLIGHT".
        /// </summary>
        PinkLight = 126,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_PM_MITEM_HIGHLIGHT".
        /// </summary>
        PmMItemHighlight = 127,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_SCRIPT_VARIABLE".
        /// </summary>
        ScriptVariable = 128,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_YOGA".
        /// </summary>
        Yoga = 129,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_TENNIS".
        /// </summary>
        Tennis = 130,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_GOLF".
        /// </summary>
        Golf = 131,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_SHOOTING_RANGE".
        /// </summary>
        ShootingRange = 132,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_FLIGHT_SCHOOL".
        /// </summary>
        FlightSchool = 133,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_NORTH_BLUE".
        /// </summary>
        NorthBlue = 134,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_SOCIAL_CLUB".
        /// </summary>
        SocialClub = 135,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_PLATFORM_BLUE".
        /// </summary>
        PlatformBlue = 136,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_PLATFORM_GREEN".
        /// </summary>
        PlatformGreen = 137,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_PLATFORM_GREY".
        /// </summary>
        PlatformGrey = 138,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_FACEBOOK_BLUE".
        /// </summary>
        FacebookBlue = 139,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_INGAME_BG".
        /// </summary>
        InGameBackground = 140,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_DARTS".
        /// </summary>
        Darts = 141,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_WAYPOINT".
        /// </summary>
        Waypoint = 142,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_MICHAEL".
        /// </summary>
        Michael = 143,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_FRANKLIN".
        /// </summary>
        Franklin = 144,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_TREVOR".
        /// </summary>
        Trevor = 145,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_GOLF_P1".
        /// </summary>
        GolfP1 = 146,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_GOLF_P2".
        /// </summary>
        GolfP2 = 147,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_GOLF_P3".
        /// </summary>
        GolfP3 = 148,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_GOLF_P4".
        /// </summary>
        GolfP4 = 149,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_WAYPOINTLIGHT".
        /// </summary>
        WaypointLight = 150,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_WAYPOINTDARK".
        /// </summary>
        WaypointDark = 151,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_PANEL_LIGHT".
        /// </summary>
        PanelLight = 152,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_MICHAEL_DARK".
        /// </summary>
        MichaelDark = 153,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_FRANKLIN_DARK".
        /// </summary>
        FranklinDark = 154,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_TREVOR_DARK".
        /// </summary>
        TrevorDark = 155,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_OBJECTIVE_ROUTE".
        /// </summary>
        ObjectiveRoute = 156,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_PAUSEMAP_TINT".
        /// </summary>
        PauseMapTint = 157,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_PAUSE_DESELECT".
        /// </summary>
        PauseDeselect = 158,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_PM_WEAPONS_PURCHASABLE".
        /// </summary>
        PmWeaponsPurchasable = 159,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_PM_WEAPONS_LOCKED".
        /// </summary>
        PmWeaponsLocked = 160,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_END_SCREEN_BG".
        /// </summary>
        EndScreenBackground = 161,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_CHOP".
        /// </summary>
        Chop = 162,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_PAUSEMAP_TINT_HALF".
        /// </summary>
        PauseMapTintHalf = 163,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_NORTH_BLUE_OFFICIAL".
        /// </summary>
        NorthBlueOfficial = 164,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_SCRIPT_VARIABLE_2".
        /// </summary>
        ScriptVariable2 = 165,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_H".
        /// </summary>
        H = 166,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_HDARK".
        /// </summary>
        HDark = 167,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_T".
        /// </summary>
        T = 168,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_TDARK".
        /// </summary>
        TDark = 169,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_HSHARD".
        /// </summary>
        HShard = 170,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_CONTROLLER_MICHAEL".
        /// </summary>
        ControllerMichael = 171,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_CONTROLLER_FRANKLIN".
        /// </summary>
        ControllerFranklin = 172,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_CONTROLLER_TREVOR".
        /// </summary>
        ControllerTrevor = 173,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_CONTROLLER_CHOP".
        /// </summary>
        ControllerChop = 174,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_VIDEO_EDITOR_VIDEO".
        /// </summary>
        VideoEditorVideo = 175,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_VIDEO_EDITOR_AUDIO".
        /// </summary>
        VideoEditorAudio = 176,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_VIDEO_EDITOR_TEXT".
        /// </summary>
        VideoEditorText = 177,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_HB_BLUE".
        /// </summary>
        HbBlue = 178,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_HB_YELLOW".
        /// </summary>
        HbYellow = 179,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_VIDEO_EDITOR_SCORE".
        /// </summary>
        VideoEditorScore = 180,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_VIDEO_EDITOR_AUDIO_FADEOUT".
        /// </summary>
        VideoEditorAudioFadeout = 181,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_VIDEO_EDITOR_TEXT_FADEOUT".
        /// </summary>
        VideoEditorTextFadeout = 182,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_VIDEO_EDITOR_SCORE_FADEOUT".
        /// </summary>
        VideoEditorScoreFadeout = 183,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_HEIST_BACKGROUND".
        /// </summary>
        HeistBackground = 184,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_VIDEO_EDITOR_AMBIENT".
        /// </summary>
        VideoEditorAmbient = 185,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_VIDEO_EDITOR_AMBIENT_FADEOUT".
        /// </summary>
        VideoEditorAmbientFadeout = 186,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_GB".
        /// </summary>
        Gb = 187,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_G".
        /// </summary>
        G = 188,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_B".
        /// </summary>
        B = 189,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_LOW_FLOW".
        /// </summary>
        LowFlow = 190,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_LOW_FLOW_DARK".
        /// </summary>
        LowFlowDark = 191,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_G1".
        /// </summary>
        G1 = 192,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_G2".
        /// </summary>
        G2 = 193,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_G3".
        /// </summary>
        G3 = 194,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_G4".
        /// </summary>
        G4 = 195,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_G5".
        /// </summary>
        G5 = 196,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_G6".
        /// </summary>
        G6 = 197,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_G7".
        /// </summary>
        G7 = 198,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_G8".
        /// </summary>
        G8 = 199,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_G9".
        /// </summary>
        G9 = 200,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_G10".
        /// </summary>
        G10 = 201,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_G11".
        /// </summary>
        G11 = 202,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_G12".
        /// </summary>
        G12 = 203,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_G13".
        /// </summary>
        G13 = 204,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_G14".
        /// </summary>
        G14 = 205,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_G15".
        /// </summary>
        G15 = 206,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_ADVERSARY".
        /// </summary>
        Adversary = 207,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_DEGEN_RED".
        /// </summary>
        DegenRed = 208,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_DEGEN_YELLOW".
        /// </summary>
        DegenYellow = 209,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_DEGEN_GREEN".
        /// </summary>
        DegenGreen = 210,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_DEGEN_CYAN".
        /// </summary>
        DegenCyan = 211,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_DEGEN_BLUE".
        /// </summary>
        DegenBlue = 212,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_DEGEN_MAGENTA".
        /// </summary>
        DegenMagenta = 213,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_STUNT_1".
        /// </summary>
        Stunt1 = 214,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_STUNT_2".
        /// </summary>
        Stunt2 = 215,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_SPECIAL_RACE_SERIES".
        /// </summary>
        SpecialRaceSeries = 216,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_SPECIAL_RACE_SERIES_DARK".
        /// </summary>
        SpecialRaceSeriesDark = 217,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_CS".
        /// </summary>
        Cs = 218,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_CS_DARK".
        /// </summary>
        CsDark = 219,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_TECH_GREEN".
        /// </summary>
        TechGreen = 220,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_TECH_GREEN_DARK".
        /// </summary>
        TechGreenDark = 221,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_TECH_RED".
        /// </summary>
        TechRed = 222,
        /// <summary>
        /// Represents the HUD color "HUD_COLOUR_TECH_GREEN_VERY_DARK".
        /// </summary>
        TechGreenVeryDark = 223,
    }

    /// <summary>
    /// Contains extensions methods for <see cref="HudColor"/>.
    /// </summary>
    public static class HudColorExtensions
    {
        /// <summary>
        /// Gets the RGBA value of the specified HUD color.
        /// </summary>
        ///<param name="hudColor">The HUD color.</param>
        public static Color GetColor(this HudColor hudColor)
        {
            N.GetHudColour((int)hudColor, out int r, out int g, out int b, out int a);
            return Color.FromArgb(a, r, g, b);
        }

        /// <summary>
        /// Sets the RGBA value of the specified HUD color.
        /// </summary>
        /// <param name="hudColor">The HUD color.</param>
        /// <param name="color">The new color value.</param>
        public static void SetColor(this HudColor hudColor, Color color) => N.ReplaceHudColourWithRgba((int)hudColor, color.R, color.G, color.B, color.A);

        /// <summary>
        /// Gets the real name of the specified HUD color. For example, for <see cref="HudColor.PureWhite"/> it will return "HUD_COLOUR_PURE_WHITE".
        /// </summary>
        /// <remarks>
        /// One of the uses of the returned name is changing the color of the text from notifications, help messages or subtitles.
        /// <code lang="C#" title="Usage example in a notification">
        /// Game.DisplayHelp($"~{HudColor.Red.GetName()}~Now this text is red!");
        /// </code>
        /// </remarks>
        /// <param name="hudColor">The HUD color.</param>
        public static string GetName(this HudColor hudColor) => hudColor switch
        {
            HudColor.PureWhite => "HUD_COLOUR_PURE_WHITE",
            HudColor.White => "HUD_COLOUR_WHITE",
            HudColor.Black => "HUD_COLOUR_BLACK",
            HudColor.Grey => "HUD_COLOUR_GREY",
            HudColor.GreyLight => "HUD_COLOUR_GREYLIGHT",
            HudColor.GreyDark => "HUD_COLOUR_GREYDARK",
            HudColor.Red => "HUD_COLOUR_RED",
            HudColor.RedLight => "HUD_COLOUR_REDLIGHT",
            HudColor.RedDark => "HUD_COLOUR_REDDARK",
            HudColor.Blue => "HUD_COLOUR_BLUE",
            HudColor.BlueLight => "HUD_COLOUR_BLUELIGHT",
            HudColor.BlueDark => "HUD_COLOUR_BLUEDARK",
            HudColor.Yellow => "HUD_COLOUR_YELLOW",
            HudColor.YellowLight => "HUD_COLOUR_YELLOWLIGHT",
            HudColor.YellowDark => "HUD_COLOUR_YELLOWDARK",
            HudColor.Orange => "HUD_COLOUR_ORANGE",
            HudColor.OrangeLight => "HUD_COLOUR_ORANGELIGHT",
            HudColor.OrangeDark => "HUD_COLOUR_ORANGEDARK",
            HudColor.Green => "HUD_COLOUR_GREEN",
            HudColor.GreenLight => "HUD_COLOUR_GREENLIGHT",
            HudColor.GreenDark => "HUD_COLOUR_GREENDARK",
            HudColor.Purple => "HUD_COLOUR_PURPLE",
            HudColor.PurpleLight => "HUD_COLOUR_PURPLELIGHT",
            HudColor.PurpleDark => "HUD_COLOUR_PURPLEDARK",
            HudColor.Pink => "HUD_COLOUR_PINK",
            HudColor.RadarHealth => "HUD_COLOUR_RADAR_HEALTH",
            HudColor.RadarArmour => "HUD_COLOUR_RADAR_ARMOUR",
            HudColor.RadarDamage => "HUD_COLOUR_RADAR_DAMAGE",
            HudColor.NetPlayer1 => "HUD_COLOUR_NET_PLAYER1",
            HudColor.NetPlayer2 => "HUD_COLOUR_NET_PLAYER2",
            HudColor.NetPlayer3 => "HUD_COLOUR_NET_PLAYER3",
            HudColor.NetPlayer4 => "HUD_COLOUR_NET_PLAYER4",
            HudColor.NetPlayer5 => "HUD_COLOUR_NET_PLAYER5",
            HudColor.NetPlayer6 => "HUD_COLOUR_NET_PLAYER6",
            HudColor.NetPlayer7 => "HUD_COLOUR_NET_PLAYER7",
            HudColor.NetPlayer8 => "HUD_COLOUR_NET_PLAYER8",
            HudColor.NetPlayer9 => "HUD_COLOUR_NET_PLAYER9",
            HudColor.NetPlayer10 => "HUD_COLOUR_NET_PLAYER10",
            HudColor.NetPlayer11 => "HUD_COLOUR_NET_PLAYER11",
            HudColor.NetPlayer12 => "HUD_COLOUR_NET_PLAYER12",
            HudColor.NetPlayer13 => "HUD_COLOUR_NET_PLAYER13",
            HudColor.NetPlayer14 => "HUD_COLOUR_NET_PLAYER14",
            HudColor.NetPlayer15 => "HUD_COLOUR_NET_PLAYER15",
            HudColor.NetPlayer16 => "HUD_COLOUR_NET_PLAYER16",
            HudColor.NetPlayer17 => "HUD_COLOUR_NET_PLAYER17",
            HudColor.NetPlayer18 => "HUD_COLOUR_NET_PLAYER18",
            HudColor.NetPlayer19 => "HUD_COLOUR_NET_PLAYER19",
            HudColor.NetPlayer20 => "HUD_COLOUR_NET_PLAYER20",
            HudColor.NetPlayer21 => "HUD_COLOUR_NET_PLAYER21",
            HudColor.NetPlayer22 => "HUD_COLOUR_NET_PLAYER22",
            HudColor.NetPlayer23 => "HUD_COLOUR_NET_PLAYER23",
            HudColor.NetPlayer24 => "HUD_COLOUR_NET_PLAYER24",
            HudColor.NetPlayer25 => "HUD_COLOUR_NET_PLAYER25",
            HudColor.NetPlayer26 => "HUD_COLOUR_NET_PLAYER26",
            HudColor.NetPlayer27 => "HUD_COLOUR_NET_PLAYER27",
            HudColor.NetPlayer28 => "HUD_COLOUR_NET_PLAYER28",
            HudColor.NetPlayer29 => "HUD_COLOUR_NET_PLAYER29",
            HudColor.NetPlayer30 => "HUD_COLOUR_NET_PLAYER30",
            HudColor.NetPlayer31 => "HUD_COLOUR_NET_PLAYER31",
            HudColor.NetPlayer32 => "HUD_COLOUR_NET_PLAYER32",
            HudColor.SimpleBlipDefault => "HUD_COLOUR_SIMPLEBLIP_DEFAULT",
            HudColor.MenuBlue => "HUD_COLOUR_MENU_BLUE",
            HudColor.MenuGreyLight => "HUD_COLOUR_MENU_GREY_LIGHT",
            HudColor.MenuBlueExtraDark => "HUD_COLOUR_MENU_BLUE_EXTRA_DARK",
            HudColor.MenuYellow => "HUD_COLOUR_MENU_YELLOW",
            HudColor.MenuYellowDark => "HUD_COLOUR_MENU_YELLOW_DARK",
            HudColor.MenuGreen => "HUD_COLOUR_MENU_GREEN",
            HudColor.MenuGrey => "HUD_COLOUR_MENU_GREY",
            HudColor.MenuGreyDark => "HUD_COLOUR_MENU_GREY_DARK",
            HudColor.MenuHighlight => "HUD_COLOUR_MENU_HIGHLIGHT",
            HudColor.MenuStandard => "HUD_COLOUR_MENU_STANDARD",
            HudColor.MenuDimmed => "HUD_COLOUR_MENU_DIMMED",
            HudColor.MenuExtraDimmed => "HUD_COLOUR_MENU_EXTRA_DIMMED",
            HudColor.BriefTitle => "HUD_COLOUR_BRIEF_TITLE",
            HudColor.MidGreyMultiplayer => "HUD_COLOUR_MID_GREY_MP",
            HudColor.NetPlayer1Dark => "HUD_COLOUR_NET_PLAYER1_DARK",
            HudColor.NetPlayer2Dark => "HUD_COLOUR_NET_PLAYER2_DARK",
            HudColor.NetPlayer3Dark => "HUD_COLOUR_NET_PLAYER3_DARK",
            HudColor.NetPlayer4Dark => "HUD_COLOUR_NET_PLAYER4_DARK",
            HudColor.NetPlayer5Dark => "HUD_COLOUR_NET_PLAYER5_DARK",
            HudColor.NetPlayer6Dark => "HUD_COLOUR_NET_PLAYER6_DARK",
            HudColor.NetPlayer7Dark => "HUD_COLOUR_NET_PLAYER7_DARK",
            HudColor.NetPlayer8Dark => "HUD_COLOUR_NET_PLAYER8_DARK",
            HudColor.NetPlayer9Dark => "HUD_COLOUR_NET_PLAYER9_DARK",
            HudColor.NetPlayer10Dark => "HUD_COLOUR_NET_PLAYER10_DARK",
            HudColor.NetPlayer11Dark => "HUD_COLOUR_NET_PLAYER11_DARK",
            HudColor.NetPlayer12Dark => "HUD_COLOUR_NET_PLAYER12_DARK",
            HudColor.NetPlayer13Dark => "HUD_COLOUR_NET_PLAYER13_DARK",
            HudColor.NetPlayer14Dark => "HUD_COLOUR_NET_PLAYER14_DARK",
            HudColor.NetPlayer15Dark => "HUD_COLOUR_NET_PLAYER15_DARK",
            HudColor.NetPlayer16Dark => "HUD_COLOUR_NET_PLAYER16_DARK",
            HudColor.NetPlayer17Dark => "HUD_COLOUR_NET_PLAYER17_DARK",
            HudColor.NetPlayer18Dark => "HUD_COLOUR_NET_PLAYER18_DARK",
            HudColor.NetPlayer19Dark => "HUD_COLOUR_NET_PLAYER19_DARK",
            HudColor.NetPlayer20Dark => "HUD_COLOUR_NET_PLAYER20_DARK",
            HudColor.NetPlayer21Dark => "HUD_COLOUR_NET_PLAYER21_DARK",
            HudColor.NetPlayer22Dark => "HUD_COLOUR_NET_PLAYER22_DARK",
            HudColor.NetPlayer23Dark => "HUD_COLOUR_NET_PLAYER23_DARK",
            HudColor.NetPlayer24Dark => "HUD_COLOUR_NET_PLAYER24_DARK",
            HudColor.NetPlayer25Dark => "HUD_COLOUR_NET_PLAYER25_DARK",
            HudColor.NetPlayer26Dark => "HUD_COLOUR_NET_PLAYER26_DARK",
            HudColor.NetPlayer27Dark => "HUD_COLOUR_NET_PLAYER27_DARK",
            HudColor.NetPlayer28Dark => "HUD_COLOUR_NET_PLAYER28_DARK",
            HudColor.NetPlayer29Dark => "HUD_COLOUR_NET_PLAYER29_DARK",
            HudColor.NetPlayer30Dark => "HUD_COLOUR_NET_PLAYER30_DARK",
            HudColor.NetPlayer31Dark => "HUD_COLOUR_NET_PLAYER31_DARK",
            HudColor.NetPlayer32Dark => "HUD_COLOUR_NET_PLAYER32_DARK",
            HudColor.Bronze => "HUD_COLOUR_BRONZE",
            HudColor.Silver => "HUD_COLOUR_SILVER",
            HudColor.Gold => "HUD_COLOUR_GOLD",
            HudColor.Platinum => "HUD_COLOUR_PLATINUM",
            HudColor.Gang1 => "HUD_COLOUR_GANG1",
            HudColor.Gang2 => "HUD_COLOUR_GANG2",
            HudColor.Gang3 => "HUD_COLOUR_GANG3",
            HudColor.Gang4 => "HUD_COLOUR_GANG4",
            HudColor.SameCrew => "HUD_COLOUR_SAME_CREW",
            HudColor.Freemode => "HUD_COLOUR_FREEMODE",
            HudColor.PauseBackground => "HUD_COLOUR_PAUSE_BG",
            HudColor.Friendly => "HUD_COLOUR_FRIENDLY",
            HudColor.Enemy => "HUD_COLOUR_ENEMY",
            HudColor.Location => "HUD_COLOUR_LOCATION",
            HudColor.Pickup => "HUD_COLOUR_PICKUP",
            HudColor.PauseSingleplayer => "HUD_COLOUR_PAUSE_SINGLEPLAYER",
            HudColor.FreemodeDark => "HUD_COLOUR_FREEMODE_DARK",
            HudColor.InactiveMission => "HUD_COLOUR_INACTIVE_MISSION",
            HudColor.Damage => "HUD_COLOUR_DAMAGE",
            HudColor.PinkLight => "HUD_COLOUR_PINKLIGHT",
            HudColor.PmMItemHighlight => "HUD_COLOUR_PM_MITEM_HIGHLIGHT",
            HudColor.ScriptVariable => "HUD_COLOUR_SCRIPT_VARIABLE",
            HudColor.Yoga => "HUD_COLOUR_YOGA",
            HudColor.Tennis => "HUD_COLOUR_TENNIS",
            HudColor.Golf => "HUD_COLOUR_GOLF",
            HudColor.ShootingRange => "HUD_COLOUR_SHOOTING_RANGE",
            HudColor.FlightSchool => "HUD_COLOUR_FLIGHT_SCHOOL",
            HudColor.NorthBlue => "HUD_COLOUR_NORTH_BLUE",
            HudColor.SocialClub => "HUD_COLOUR_SOCIAL_CLUB",
            HudColor.PlatformBlue => "HUD_COLOUR_PLATFORM_BLUE",
            HudColor.PlatformGreen => "HUD_COLOUR_PLATFORM_GREEN",
            HudColor.PlatformGrey => "HUD_COLOUR_PLATFORM_GREY",
            HudColor.FacebookBlue => "HUD_COLOUR_FACEBOOK_BLUE",
            HudColor.InGameBackground => "HUD_COLOUR_INGAME_BG",
            HudColor.Darts => "HUD_COLOUR_DARTS",
            HudColor.Waypoint => "HUD_COLOUR_WAYPOINT",
            HudColor.Michael => "HUD_COLOUR_MICHAEL",
            HudColor.Franklin => "HUD_COLOUR_FRANKLIN",
            HudColor.Trevor => "HUD_COLOUR_TREVOR",
            HudColor.GolfP1 => "HUD_COLOUR_GOLF_P1",
            HudColor.GolfP2 => "HUD_COLOUR_GOLF_P2",
            HudColor.GolfP3 => "HUD_COLOUR_GOLF_P3",
            HudColor.GolfP4 => "HUD_COLOUR_GOLF_P4",
            HudColor.WaypointLight => "HUD_COLOUR_WAYPOINTLIGHT",
            HudColor.WaypointDark => "HUD_COLOUR_WAYPOINTDARK",
            HudColor.PanelLight => "HUD_COLOUR_PANEL_LIGHT",
            HudColor.MichaelDark => "HUD_COLOUR_MICHAEL_DARK",
            HudColor.FranklinDark => "HUD_COLOUR_FRANKLIN_DARK",
            HudColor.TrevorDark => "HUD_COLOUR_TREVOR_DARK",
            HudColor.ObjectiveRoute => "HUD_COLOUR_OBJECTIVE_ROUTE",
            HudColor.PauseMapTint => "HUD_COLOUR_PAUSEMAP_TINT",
            HudColor.PauseDeselect => "HUD_COLOUR_PAUSE_DESELECT",
            HudColor.PmWeaponsPurchasable => "HUD_COLOUR_PM_WEAPONS_PURCHASABLE",
            HudColor.PmWeaponsLocked => "HUD_COLOUR_PM_WEAPONS_LOCKED",
            HudColor.EndScreenBackground => "HUD_COLOUR_END_SCREEN_BG",
            HudColor.Chop => "HUD_COLOUR_CHOP",
            HudColor.PauseMapTintHalf => "HUD_COLOUR_PAUSEMAP_TINT_HALF",
            HudColor.NorthBlueOfficial => "HUD_COLOUR_NORTH_BLUE_OFFICIAL",
            HudColor.ScriptVariable2 => "HUD_COLOUR_SCRIPT_VARIABLE_2",
            HudColor.H => "HUD_COLOUR_H",
            HudColor.HDark => "HUD_COLOUR_HDARK",
            HudColor.T => "HUD_COLOUR_T",
            HudColor.TDark => "HUD_COLOUR_TDARK",
            HudColor.HShard => "HUD_COLOUR_HSHARD",
            HudColor.ControllerMichael => "HUD_COLOUR_CONTROLLER_MICHAEL",
            HudColor.ControllerFranklin => "HUD_COLOUR_CONTROLLER_FRANKLIN",
            HudColor.ControllerTrevor => "HUD_COLOUR_CONTROLLER_TREVOR",
            HudColor.ControllerChop => "HUD_COLOUR_CONTROLLER_CHOP",
            HudColor.VideoEditorVideo => "HUD_COLOUR_VIDEO_EDITOR_VIDEO",
            HudColor.VideoEditorAudio => "HUD_COLOUR_VIDEO_EDITOR_AUDIO",
            HudColor.VideoEditorText => "HUD_COLOUR_VIDEO_EDITOR_TEXT",
            HudColor.HbBlue => "HUD_COLOUR_HB_BLUE",
            HudColor.HbYellow => "HUD_COLOUR_HB_YELLOW",
            HudColor.VideoEditorScore => "HUD_COLOUR_VIDEO_EDITOR_SCORE",
            HudColor.VideoEditorAudioFadeout => "HUD_COLOUR_VIDEO_EDITOR_AUDIO_FADEOUT",
            HudColor.VideoEditorTextFadeout => "HUD_COLOUR_VIDEO_EDITOR_TEXT_FADEOUT",
            HudColor.VideoEditorScoreFadeout => "HUD_COLOUR_VIDEO_EDITOR_SCORE_FADEOUT",
            HudColor.HeistBackground => "HUD_COLOUR_HEIST_BACKGROUND",
            HudColor.VideoEditorAmbient => "HUD_COLOUR_VIDEO_EDITOR_AMBIENT",
            HudColor.VideoEditorAmbientFadeout => "HUD_COLOUR_VIDEO_EDITOR_AMBIENT_FADEOUT",
            HudColor.Gb => "HUD_COLOUR_GB",
            HudColor.G => "HUD_COLOUR_G",
            HudColor.B => "HUD_COLOUR_B",
            HudColor.LowFlow => "HUD_COLOUR_LOW_FLOW",
            HudColor.LowFlowDark => "HUD_COLOUR_LOW_FLOW_DARK",
            HudColor.G1 => "HUD_COLOUR_G1",
            HudColor.G2 => "HUD_COLOUR_G2",
            HudColor.G3 => "HUD_COLOUR_G3",
            HudColor.G4 => "HUD_COLOUR_G4",
            HudColor.G5 => "HUD_COLOUR_G5",
            HudColor.G6 => "HUD_COLOUR_G6",
            HudColor.G7 => "HUD_COLOUR_G7",
            HudColor.G8 => "HUD_COLOUR_G8",
            HudColor.G9 => "HUD_COLOUR_G9",
            HudColor.G10 => "HUD_COLOUR_G10",
            HudColor.G11 => "HUD_COLOUR_G11",
            HudColor.G12 => "HUD_COLOUR_G12",
            HudColor.G13 => "HUD_COLOUR_G13",
            HudColor.G14 => "HUD_COLOUR_G14",
            HudColor.G15 => "HUD_COLOUR_G15",
            HudColor.Adversary => "HUD_COLOUR_ADVERSARY",
            HudColor.DegenRed => "HUD_COLOUR_DEGEN_RED",
            HudColor.DegenYellow => "HUD_COLOUR_DEGEN_YELLOW",
            HudColor.DegenGreen => "HUD_COLOUR_DEGEN_GREEN",
            HudColor.DegenCyan => "HUD_COLOUR_DEGEN_CYAN",
            HudColor.DegenBlue => "HUD_COLOUR_DEGEN_BLUE",
            HudColor.DegenMagenta => "HUD_COLOUR_DEGEN_MAGENTA",
            HudColor.Stunt1 => "HUD_COLOUR_STUNT_1",
            HudColor.Stunt2 => "HUD_COLOUR_STUNT_2",
            HudColor.SpecialRaceSeries => "HUD_COLOUR_SPECIAL_RACE_SERIES",
            HudColor.SpecialRaceSeriesDark => "HUD_COLOUR_SPECIAL_RACE_SERIES_DARK",
            HudColor.Cs => "HUD_COLOUR_CS",
            HudColor.CsDark => "HUD_COLOUR_CS_DARK",
            HudColor.TechGreen => "HUD_COLOUR_TECH_GREEN",
            HudColor.TechGreenDark => "HUD_COLOUR_TECH_GREEN_DARK",
            HudColor.TechRed => "HUD_COLOUR_TECH_RED",
            HudColor.TechGreenVeryDark => "HUD_COLOUR_TECH_GREEN_VERY_DARK",
            _ => throw new System.ArgumentOutOfRangeException(nameof(hudColor), hudColor, $"Invalid {nameof(HudColor)}"),
        };
    }
}
